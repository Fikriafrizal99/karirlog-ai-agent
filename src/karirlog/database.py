...
class Database:
    ...
    def save_analysis(self, job_id: int, result: AnalysisResult) -> None:
        self.connection.execute(
            ...
            (job_id, score, decision, analysis_mode, provider, model, confidence,
             ...
             transferable_strengths, red_flags, reasons)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                job_id,
    ...
    def save_or_update_application(
        ...
        if row is None:
            self.save_application(job_id, status, mode, output_path, package)
            return True
        self.connection.execute(
            """
